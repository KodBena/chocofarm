...
int main(int argc, char** argv) {
    ...
    cfg.n_sims = chocofarm::SimBudget{12};   // match wire-bench's default so the axes are compared at the SAME search budget
    ...
    if (net->in_dim() != static_cast<int>(fb.dim().value())) {
        std::cerr << "local-mlp-bench: FATAL: published net in_dim=" << net->in_dim()
                  << " != env feature_dim=" << fb.dim().value() << " (dimension mismatch — wrong weights?)\n";
        return 1;
    }
