...
class ScriptedSource final : public chocofarm::WorldSource {
  public:
    ...
    double playout_value(const chocofarm::Loc&, const std::vector<uint32_t>&,
        ...
        if (vals_.empty()) throw std::runtime_error("nmcs_dump: empty scripted playout table");
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        cfg.level = opt(argc, argv, "--level") ? std::atoi(opt(argc, argv, "--level")) : 1;
        cfg.cand_det = opt(argc, argv, "--cand-det") ? std::atoi(opt(argc, argv, "--cand-det")) : 4;
        cfg.cand_tre = opt(argc, argv, "--cand-tre") ? std::atoi(opt(argc, argv, "--cand-tre")) : 4;
        cfg.step_samples =
            opt(argc, argv, "--step-samples") ? std::atoi(opt(argc, argv, "--step-samples")) : 2;
        cfg.max_steps = opt(argc, argv, "--max-steps") ? std::atoi(opt(argc, argv, "--max-steps")) : 24;
        // playout_samples is irrelevant here (the scripted source ignores it), but kept default.
        double lam = opt(argc, argv, "--lam") ? std::atof(opt(argc, argv, "--lam")) : 0.1;
        ...
        if (const char* pref = opt(argc, argv, "--prefix")) {
        ...
        std::cerr << "nmcs-dump: FATAL: " << e.what() << "\n";
        return 1;
    }
}
