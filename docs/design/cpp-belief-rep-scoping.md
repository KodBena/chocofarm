...
## PERF FOLLOW-UP NOTE (2026-06-17 — amend-by-append, ADR-0005 Rule 8)

A K=128 client profile (gate ON, bitset live) showed the belief-math collapsed to ~20% (the bitset arm working as designed), surfacing two NEW top costs the bitset rep introduced or left: (a) the per-node belief-copy heap alloc (~14.5% of the malloc/free family — the `BitsetBelief::bits` vector allocating ~2 KiB on every descent-local copy), and (b) `std::_Rb_tree_increment` (~10% — the `GumbelNode`/`ISMCTSNode` per-action `std::map<int,.>` stats). Two BIT-EXACT storage/container refactors attacked them (both BYTE-IDENTICAL — they change storage/containers, NOT values or order; the flat-vs-bitset A/B + the full gumbel/ismcts/nmcs parity at 25 passed / 2 skipped are the gates, all UNCHANGED):
