...
> **Scope correction (2026-06-15, post cross-audit synthesis — see Appendix E).** This claim is scoped to `main@cfce276`, the branch this audit read. The in-flight branch `feat/hp-registry` introduces a **typed-schema config SSOT** (`chocofarm/hp/schema.py`) + a **redis-backed live-read registry** (`chocofarm/hp/registry.py`) + `chocofarm/config.py` — none present on `main` *(byte-verified)*. That layer is a *partial cure*: it makes per-iteration fields (e.g. `n_step`/`td_lambda`) HOT via a per-iteration snapshot with a loud RESTART-drift refusal, but its own design keeps **`lr`/`l2`/search-width RESTART (still baked into `optax.adam` at construction)**. So "no central config object" is false on `feat/hp-registry`, but the core finding — *the experimentation levers that matter (lr, search width) are still frozen* — stands there too, and is exactly what R13 targets. The registry should land to `main` and then be completed, not re-invented.
...
> **Qualification (2026-06-15, post cross-audit synthesis — see Appendix E).** Disciplined *and orphaned*: nothing outside `analysis/` imports it *(grep-verified)* — it is disconnected from the live pipeline. The coupling cost the prior audit names: the solver's cluster definitions (`decomp.py`) are hardcoded rather than read from the analyzer, so the two are unrelated implementations of one intent. The praise holds for the code's internal quality; the disposition (wire it into the env's cluster discovery, or relocate to a `tools/`-style home) is a maintainer call (see §11).
