...
> **Witness 7 (2026-06-24, later — the question that broke Witness 6: "does `overcommit_sweep` do numpy?" It does NOT. The inflation was reducible IN JAX, and numpy was a confounded comparison.** The maintainer's challenge: overcommit's 140k baseline runs the **staged JAX forward** (`build_staged_forward` via the lowlatency AOT dispatcher; `InferenceServer._stages_params = forward_fn is jit_forward_core`), **not** numpy — so Witness 6's numpy route changed the *comparison basis* and never answered why *our* jax forward inflates in-serve vs *their* jax forward (apples-to-apples). Two corrections fell out:
>
> 1. **Witness 6's numpy A/B was confounded.** The harness pins `OMP_NUM_THREADS=1` for the numpy arm (line 45) but never pinned the jax arm's XLA Eigen pool — so it compared *single-thread-BLAS numpy* vs *unpinned multi-thread-Eigen jax*, not XLA-dispatch-vs-numpy. (The Eigen pin itself — `chocofarm.config`'s `--xla_cpu_multi_thread_eigen=false`, which the production server gets by import and the clean-room tlab server did not — is worth only **~5%** here: 1442→1373 µs single-reading. The documented production fix, but a minor lever.)
> 2. **The inflation was REDUCIBLE in JAX — it was not "irreducible XLA launch latency."** Wired the *real* production forwards into the tlab server as diagnostic arms (`--forward prod` = `jit_forward_core`; `--forward staged` = `build_staged_forward`; commit `6deb40c`, a deliberate measurement-only clean-room boundary crossing) and ran a controlled in-serve A/B — single-thread, **all** pinned, 256-padded, **3 interleaved replicates**, median compute µs/forward:
>
> | forward (in-serve, pinned, 256-pad) | median µs/fwd | vs old jax |
> | --- | ---: | ---: |
> | **old jax** (`_forward_both`: eager `jnp.asarray(x)` convert + `(v,logits)` tuple, **two** dispatches/pulls) | 1386 | — |
> | `prod` (`jit_forward_core`, one packed pull, host params) | 1228 | −11% |
> | `staged` (`build_staged_forward`, one pull, device params — the **actual** 140k forward) | 1187 | −14% |
> | **new jax** (`_forward_packed`: cast **folded into the jit**, `[v|logits]` concatenated, **one** pull) | **1160** | **−16.3%** |
>
> So Witness 6's "the d2h two-pull is dead (31 µs)" was right about the *pull* but missed the real cost: the **eager `jnp.asarray(x)` host→device convert was a SECOND XLA dispatch** (the 331 µs "h2d" phase) ahead of the jit — *two* dispatches, not one. The production forward folds the cast into the jit (one dispatch) and packs `[v|logits]` (one pull). Adopting that pattern in pure clean-room JAX (`_forward_packed`, commit `6984b24`, the same `forward_core` SSOT — ADR-0012 P6) drops new jax to **1160 µs/fwd, matching `staged` and beating `prod`** — the full envelope captured **in JAX, no numpy, no `chocofarm` dependency.**
>
> **THROUGHPUT conversion WITNESSED at 3 generators** (the 4-gen run *appeared* sandbox-blocked — see Witness 8, it was not — but 3 producers survive and reach ~65% util). Single-thread, interleaved 2-rep median leaf-rows/s: **old 90,968 → new 96,392 = +5.9%** (`onepull_3gen_ab`, readings 33–36). Partial, as expected — at 65% util the server is not yet forward-saturated, so only part of the 16% envelope converts; the full conversion needs the forward-bound 4-gen regime. **State:** the forward gap is now correctly attributed (our wrapper, not XLA-vs-numpy, not threading, not staging) and **fixed in JAX**; +5.9% at 3-gen is banked; the >140k overshoot remains unwitnessed. numpy stays a real but *different-basis* lever (overcommit could adopt it too; unlikely to generalize off this tiny CPU MLP) — **not** the bridge. (DB: finding **#8** supersedes #7.)

> **Witness 8 (2026-06-24, later — the "sandbox signal-16 4-gen kill" was a HARNESS BUG, not an environment limit; 4-gen now runs clean.** Witnesses 6–7 and findings #7–#8 carried a caveat that sustained 4-process / 4-gen runs "die with signal 16 — a sandbox CPU-quota limit." That was never instrumented; when it finally was, the server log said **`episodic_dps.sh: line 57: PYTHONPATH=throughput-lab: command not found`** — the server never started. The cause: line 57 launched the server with a leading *variable-expansion* assignment prefix, `$SRVENV PYTHONPATH=… taskset …`. **bash recognizes assignment prefixes at PARSE time on LITERAL tokens only** — a `$SRVENV` expansion is not a literal `name=value`, so it terminates the assignment scan and the *next* literal `PYTHONPATH=…` falls into command-word position; bash runs it as a command. Confirmed under bash 5.3 for **both** empty and non-empty `$SRVENV`. (The summary even records that I had the correct `env $SRVENV` form earlier and **reverted** it to the bare prefix while chasing an unrelated `exit-144` — the revert is what broke every `episodic_dps.sh` run after the `$SRVENV` prefix landed.) **Fix:** `env $SRVENV PYTHONPATH=… taskset …` (env takes the words as `VAR=VAL` args, not a shell assignment prefix), commit `0ad4e07`, guarded by a load-bearing comment. With the fix the **4-gen witness runs clean on the first try: 101,152 leaf-rows/s, 67% util, mean batch 200.6 rows** (single-thread one-pull jax, `onepull_4gen_witness`, reading 37). *(NB: `exit-144` = 128+16 is real and still hits some heavy compound bash commands like `pkill`+`git` — but it is NOT what blocked the benchmarks; that was the parse bug.)* **Lesson (`attribute-discrepancies`, ADR-0013 verify-the-artifact):** a named "environment limit" that was never instrumented was a one-line code bug — the harness's own log had the answer the whole time. DB: finding **#9** (scope `harness-server-launch-bug`, confirmed). The 4-gen regime is now UNBLOCKED — the overshoot hunt can resume against a working harness.

> **Witness 9 (2026-06-24, the headline correction — THE 140k TARGET NEVER EXISTED. It was a 2.00× window-mismatch artifact; the reference's true rate is ~69k, and our ~100k has been beating it all along.** Mandated to measure the *reference* stack's component ceilings (the right-hand column the whole investigation lacked), the reference server util/throughput would not reconcile: server-side `n_real_rows`/wall ≈ 62–76k, the bench's own `dps_per_core` ≈ 48 → ~120k via ×LPD(835), the recorded reading ≈ 140k — three numbers, up to 2× apart, and 140k unreproducible. After three attempts (wrong operating point → a `compute_s` probe that *serialized the pipeline* and halved it → non-invasive CPU, still ~62k), escalated to **ADR-0014** (consult-005) rather than a 4th self-attempt. The reviewer diagnosed a **unit mismatch**, and per **ADR-0013 Rule 5 I verified the artifact, not the claim**: a fresh `overcommit_sweep` N9 reproduced the recorded leaf count **`8,899,482` bit-identically** (it is deterministic — fixed warm-pool work), and:
>
> | divide that leaf count by… | rate | |
> | --- | ---: | --- |
> | the **measure-window** wall (62.7 s) | **141,985** | ≈ the recorded **140,578** — the artifact |
> | the **whole-call** wall (125.6 s, warmup + measure) | **70,851** | the true average rate |
>
> ratio **2.00×**. The recorded `leaf_rows_s` (readings 16/17) divided *whole-call* leaves (the server's `n_real_rows` counter spans the warmup pass *and* the measure pass — both forward real rows) by the *measure-window* wall (`dps`'s denominator). The recorded `lpd=835` is the same disease (whole-call leaves ÷ measure-window decisions; window-consistent LPD ≈ 421). **The reference's true rate at its N9 "140k" operating point is ~69–71k leaf-rows/s, and its server core is ~73–79% CPU — NOT saturated** (three independent measurements; the bottleneck is upstream, the real search + wire RTTs, not the JAX forward).
>
> **What this overturns.** The "2× gap" (Witness 3 / finding #3, "ours 70k vs overcommit 140k = 2.0×") **never existed** — the real comparison was roughly *parity then*, and our episodic stack (~100k, reading 37) **leads the real reference (~69k) by ~45% now**. The entire forward-envelope arc (Witnesses 4–8), the producer/serve-loop attribution, the numpy digression — all chased a target that was a measurement bug. The *engineering wins are real and stand on their own* (the one-pull forward −16% envelope; the `episodic_dps.sh` env-prefix fix; the isolated-ceiling decomposition C_fwd/C_serve/C_prod; the producer-vs-serve-loop attribution) — but the *goal* they were aimed at was phantom. **Nuance, not overclaimed:** the two stacks run different workloads (our full-episode MCTS vs the reference's warm-pool replay); `leaf-rows/s` is the workload-abstracting throughput metric, and on it we lead. DB: **finding #12** (`reference-140k-is-window-mismatch-artifact`, confirmed; corrected reference reading #50, ~71k / server ~75% unsaturated); readings 16/17 recommended **retracted**. **Meta-lesson:** the single most expensive defect of this multi-week effort was an un-audited *target number* — a metric computed once, in an ad-hoc scratchpad step, with mismatched numerator/denominator windows, then chased for weeks. `stamp-commit-hash-on-readings` and the new pre-registration layer guard the *experiments*; this says the **baseline you are bridging to** deserves the same artifact-level scrutiny as your own readings (verify the target, not just the attempts).
