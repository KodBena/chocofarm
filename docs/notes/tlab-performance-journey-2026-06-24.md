...
>
> **Coalescing translates to a clean 3.0× DPS win in the production-shape workload (27 → 82)** — not just the synthetic leaf-rows metric. But the deeper finding settles the correction's open sign: at 82 DPS the system is **server-compute-limited** (server 73% matmul; the ~58k leaf-rows/s server ceiling ÷ 634 LPD ≈ **~92 DPS max**), well below the **~138–184 DPS search ceiling** on this box (3–4 gen cores × the measured 46 DPS/core). So the residual bottleneck is the **server's compute, not the coupling** — meaning *any* coupling control, static or dynamic, has only ~11% headroom left (82 → ~92); the path to more DPS is a **faster server** (batch size already maxed; a GPU / lighter net / lower per-row CPython), not a smarter gate. **Production comparison — OPEN, under attribution (do NOT assume).** An earlier same-day guess here — that the gap to (190–210, 457) was a **core-count** difference (~10 cores ÷ 46 DPS/core) — is **RETRACTED**: it is the **same 4-vCPU machine**, so that inference is false. It was, once more, a speculation where attribution was the mandate (ADR-0013). The provenance, exact config, metric, and *definitions* behind (190–210, 457) are being traced to their source so the difference is **attributed, not assumed**. Hypotheses to test (not conclusions): the comparable primary metric is **leaf/s**, not DPS, with server utilization / recoverable slack as the secondary axis — and the apparent DPS gap may be a measurement *impedance* (LPD definition, executed-vs-all-decision counting, eval mocked vs real, sims/m). Until attributed, treat the 82-vs-(190/457) relationship as **unknown**.
