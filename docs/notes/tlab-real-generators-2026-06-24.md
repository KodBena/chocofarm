...
### 2. The real workload is ~~generator-bound~~ SERVER-bound (corrected)
At the asymptote the inference server's MATMUL sits at ~58% — which I read as "starved → the 3 search cores must be the bottleneck (generator-bound)." Real generation is ~16k leaf-rows/s, ~20× below the synthetic ceiling (~300k).

> **Correction (2026-06-24, `scenario_audit.py`).** The "generator-bound" label was an **inference, never a direct measurement** — I keyed on the server's *matmul %* (~58%) and a throughput plateau, and never measured the generator cores. A direct per-core probe (`measure_util`) + the scenario audit show the generators are **never CPU-saturated (18–35% busy)** — they are *reply-bound* (idle, waiting on the round-trip). The real bottleneck is the **server core** (~92–98% busy, but only ~49% of that is matmul; the other ~half is serve-path overhead — the IO thread's zmq drain/reply + a flood of tiny-batch dispatches: 242 forwards of 1 row, 466 of 8 rows). There was **no Release→production regime flip** (I had attributed one to `-march=native`); the audit proves toggling `-march=native` does NOT change the regime — both builds are server-bound (native gens 21% vs plain 35% util, same regime). `-march=native` only speeds the search modestly. **So the slack for dynamic control is the server core's ~50% non-matmul overhead (coalescing-addressable: fewer/bigger forwards), not generator idle.** This correction is exactly what the auditable toggle exists to surface.
