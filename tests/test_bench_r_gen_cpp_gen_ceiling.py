"""
tests/test_bench_r_gen_cpp_gen_ceiling.py
=========================================

The ADR-0008 RECLASSIFICATION of `R_gen` (the per-core single-generator decision rate,
decisions/s/core): a MEASURED quantity that was MIS-WIRED as an un-shrinkable `Fixed` pin, now
wired to the live C++ gen-ceiling bench (`cpp/build/chocofarm-search-runtime-bench`, source
cpp/src/search_runtime_bench.cpp) so it returns a SHRINKABLE Estimate the Neyman loop can sample
(docs/design/harmonized-estimator-interface.md §3 the PIN-now/measurable-later row + §7.A; the
defect: the bench PUNTED — `_measure_raw()` returned the seed 152, `_estimate_from_raw()` wrapped
it in `pin_estimate` -> a `Fixed`/`marginal=0`/un-fundable Estimate -> `untrusted_drive`'s
generation arm STALLED).

THE FIX, AS TESTS:
  * `_estimate_from_raw` over a per-rep decisions/s/core pool builds a SHRINKABLE `QuantileLaw`
...
tail RUNS the real bench (sole-workload, taskset -c 0): it must report ~152 dps/core and produce a
shrinkable Estimate. The estimate/bench modules live under tools/analysis/leaf_eval_bound/ (no
...
from leaf_eval_bound.benchmarks import bench_r_gen as B  # noqa: E402  — the module under test
from leaf_eval_bound.contract import estimate as E  # noqa: E402  — the contract


def _rate_pool(median: float = 152.0, sigma: float = 1.0, n: int = 8, seed: int = 3) -> list[float]:
    """A plausible per-rep decisions/s/core pool: a tight near-symmetric cluster around `median` (the
    shape the C++ bench's per-rep `serial_dps` readings actually have — verified 150.6–153.4 dps/core
    across reps). A real spread, a well-defined median."""
...
def _raw(pool: list[float]) -> dict:
    ...
    return {
        "r_gen_dps_per_core": float(np.median(pool)),
        "per_rep_dps": pool,
        "serial_dps_headline": float(np.median(pool)),
...
# --------------------------------------------------------------------------- #
def test_r_gen_estimate_is_a_shrinkable_quantile_law_not_a_fixed_pin() -> None:
    """The defect was a `Fixed` (un-shrinkable) Estimate; the fix is a `QuantileLaw` (median) with a
    BOOTSTRAP median SE. This is the ADR-0008 reclassification: R_gen is a MEASURED quantity, not a pin."""
    est = B._estimate_from_raw(_raw(_rate_pool()))
    ...
    assert est.k == 1
    assert est.theta_hat[0] == pytest.approx(float(np.median(_rate_pool())))
    assert est.is_valid()


def test_r_gen_cov_is_a_real_bootstrap_se_not_a_fixed_sigma() -> None:
    """The variance authority is a REAL bootstrap median SE in `cov` (a positive, finite spread the loop
    can shrink), NOT the seed's frozen declared σ=8 wrapped un-divided (the old `Fixed` pin's cov=[[64]])."""
    est = B._estimate_from_raw(_raw(_rate_pool(sigma=1.0, n=8)))
    boot_se = math.sqrt(float(est.cov[0, 0]))
    assert boot_se > 0.0 and math.isfinite(boot_se)
    # it is the per-rep pool's order-statistic spread (sub-dps for a tight gen-ceiling pool), NOT σ=8.
    assert boot_se < 8.0


def test_r_gen_marginal_is_negative_so_the_loop_can_fund_it() -> None:
    """THE PAYOFF (§1 D2 / §2.3): the typed `ShrinkLaw.marginal_dvar_deffort` is < 0 for the QuantileLaw
    (so the driver's `_fundability` `A_i = −marginal·n²` is > 0 -> R_gen is FUNDABLE -> the loop samples
...
def test_r_gen_quantile_law_is_self_consistent_with_cov() -> None:
...
def test_r_gen_estimate_jsonb_round_trips() -> None:
    """The measured R_gen Estimate is an exact jsonb round-trip (the §5 SSOT serialization)."""
    est = B._estimate_from_raw(_raw(_rate_pool()))
...
def test_more_reps_tightens_the_se() -> None:
    """The Neyman loop sizes R_gen's measurement by `reps`; a bigger pool gives a tighter median SE (so
    more/longer runs -> a tighter R_gen CI -> a tighter generation-arm CI — the loop's lever)."""
    se_small = math.sqrt(float(B._estimate_from_raw(_raw(_rate_pool(n=4, seed=1))).cov[0, 0]))
    se_large = math.sqrt(float(B._estimate_from_raw(_raw(_rate_pool(n=64, seed=1))).cov[0, 0]))
    assert se_large < se_small


# --------------------------------------------------------------------------- #
# 3. FAIL LOUD (ADR-0002): the C++ bench absent RAISES — never a silent seed fall-back.
# --------------------------------------------------------------------------- #
def test_missing_cpp_bench_raises_never_silently_seeds(monkeypatch) -> None:
    ...
    to the 152 seed as if measured (the punt this module removes). The seed stays the DISTRUST fallback
...
def test_get_seed_is_the_distrust_fallback_and_unchanged() -> None:
    """`get_seed()` stays the v1 152 dps/core grounding (the DISTRUST fallback the manifest seed path
    uses) — the fix changes the MEASURED path, not the seed."""
    seed = B.get_seed()
    assert seed.name == "R_gen"
    assert seed.mean == 152.0
    assert seed.unit == "decisions/s/core"


def test_non_pass_cpp_output_raises(monkeypatch) -> None:
    """ADR-0002: a bench run that does not report `RESULT: PASS serial_dps=...` (e.g. a serial/parallel
    mismatch FAIL, or truncated output) RAISES rather than fabricating a rate. Driven by stubbing the
    subprocess so no live binary is needed."""
    class _Proc:
        returncode = 0
...
# --------------------------------------------------------------------------- #
# 4. BINARY-GATED: RUN the real C++ gen-ceiling bench (sole-workload) — ~152 dps/core, shrinkable.
# --------------------------------------------------------------------------- #
...
def test_live_cpp_bench_reports_about_152_dps_per_core_and_is_shrinkable() -> None:
    ...
    must (a) report ~152 dps/core at the sims256/m24 config (the seed grounding) and (b) produce a
    ...
    # ~152 dps/core (the gen-ceiling grounding). A generous band — this is a behavioral sanity gate, not
    # a tight equivalence pin (the rate carries scheduler jitter; verified ~150.5–153.4 across reps).
    assert 130.0 <= float(est.theta_hat[0]) <= 175.0
...
def test_live_cpp_bench_cross_reads_lpd_about_500() -> None:
    ...
    the LPD=500 design pin cites (so R_gen and LPD/g_core come from ONE measurement — the §4 finding)."""
