...
_BENCH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                      "tools", "analysis", "leaf_eval_bound", "benchmarks")
for _p in (os.path.dirname(_BENCH), _BENCH):
    if _p not in sys.path:
        sys.path.insert(0, _p)

import register_benches  # noqa: E402
...
def test_discovery_finds_every_bench_on_disk() -> None:
    ...
    on_disk = {f[:-3] for f in os.listdir(_BENCH) if f.startswith("bench_") and f.endswith(".py")}
