...
#
#   Usage:  episodic_dps.sh [K] [SECONDS] [N_SIMS] [MSG_ROWS]   (defaults: 128 14 256 64)
...
K="${1:-128}"; S="${2:-14}"; NSIMS="${3:-256}"; MSG_ROWS="${4:-64}"
...
G(){ taskset -c "$1" ${2:-} "$PROD" --instance "$INST" --faces "$FACES" --endpoint "$EP" \
     --threads 1 --fibers "$K" --msg-rows "$MSG_ROWS" --episodic --no-early-exit --driver round-sync \
...
rm -rf "$TMP"
echo "EPISODIC-STATIC (sims${NSIMS}/m24, no-early-exit, 3 gens + IDLE surplus, msg-rows=$MSG_ROWS, K=$K, ${S}s)"
