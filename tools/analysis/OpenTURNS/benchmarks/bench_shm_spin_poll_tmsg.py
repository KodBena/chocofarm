"""
tools/analysis/OpenTURNS/benchmarks/bench_shm_spin_poll_tmsg.py
==============================================================
...
_DESC = ("SHM SPIN-POLL per-leaf message cost (us/leaf): the in-ring memcpy of one request row in + one "
         ...
         "The SHM variant of tmsg_us_leaf (bare ring copy, no ZMQ codec).")
...
def register_self() -> Any:
    ...
    return register_quantity(NAME, quantity="transport_msg_cost_per_leaf_shm_spin_poll", units="us",
...
if __name__ == "__main__":
    _m, _s, _u = get_seed()
    print(f"[bench_shm_spin_poll_tmsg] seed: {_m:.3f} {_u} (sigma {_s:.3f}) — first-principles "
          f"(bare ring memcpy of one req row in + one reply row out; non-binding)")
    register_self()
    print("[bench_shm_spin_poll_tmsg] registered. NOT running the live measurement (timing-sensitive); "
