...
def _measure_raw(s_leaves: int = 256, iters: int = 5000) -> dict[str, Any]:
    """The raw-pool PROVENANCE producer (the §6 Phase-4 internal helper): measure tmsg_us_leaf: time
    encode_request(S x in_dim) + decode_response(reply) over `iters`, /S. Returns {'tmsg_us_leaf',
    'encode_us', 'decode_us', 's_leaves'}. Imports the codec + numpy lazily. `measure()` wraps the seed
    into a `Fixed` Estimate; `run()` uses this dict for the raw provenance row."""
    import numpy as np
...
def measure(s_leaves: int = 256, iters: int = 5000) -> "_est.Estimate":
    """Measure tmsg_us_leaf and return its harmonized k=1 `Fixed` `Estimate` (§6 Phase 4: `measure()` returns
    the `Estimate` the bench DECLARES — a pin is a `Fixed`/declared-spread Estimate, NOT a faked pool,
    consumed directly by the driver/untrusted_drive). The raw dict is the bench's internal `_measure_raw()`
    provenance."""
