"""
tools/analysis/OpenTURNS/model_shm_spin_poll.py
===============================================

Transport variant SHM-SPIN-POLL: the leaf-eval throughput LOWER BOUND (dps) for a SHARED-
MEMORY RING transport whose serve core BUSY-POLL-SPINS an atomic head/tail counter (no
syscall, no broker), drains request rows straight out of the ring, and writes replies into a
reply ring. It composes the SAME serialized-serve-cycle structure the v1 cycle-time model
...
WHAT THIS TRANSPORT MOVES (vs the ZMQ baseline; the rest of the cycle is INVARIANT):
  * tau_io  -> `shm_spin_poll_tau_io_us`: the per-forward serial drain+scatter collapses from
    the ZMQ multipart/recv/send/codec (~20us seed) to a REPLY-RING memcpy + per-message
    counter bookkeeping (~8.8us seed at B_op=256), because the request rows land row-major in
    the ring and the staging reads them ZERO-COPY (a ring span). This is the DOMINANT lever
    the transport design controls and the binding-stage term — the top Neyman target while
    unmeasured.
  * WAKEUP  -> `shm_spin_poll_wakeup_us` (~0.1us): a cached atomic load on a dedicated burnt
    poll core, no `zmq.poll` syscall, no context switch. Reported as a SEPARATE additive cycle
    term (the brief names it distinctly); at ~0.1us it is negligible vs the ~900us+ full-bucket
    cycle, but it MAKES EXPLICIT that the spin transport pays ~0 wakeup — the property that
    distinguishes it from the syscall-wakeup variants. The fixed 1-serve-core pinning already
    dedicates this core, so spinning it costs no extra core.
  * msg-cost -> `shm_spin_poll_tmsg_us_leaf` (~0.15us/leaf): a bare in-ring memcpy, no frame
    envelope. Reported (non-binding by a wide margin), ranks LAST for the allocator.
...
shm ring feeds the SAME staged `run_microbatch` forward, so t_row is the staged slope).

THE COPY-BOTH CONTRAST (computed, honestly surfaced — NOT the headline). The shm tau_io seed
uses the ZERO-COPY request drain (the design's defining property). Whether that elision is
truly realized is the dominant uncertainty; `copy_both_contrast()` adds the SEPARATE
`shm_spin_poll_req_drain_us` term (~31us) to show the bound IF the request drain is a real
memcpy (the pessimistic arm). The Neyman allocator then ranks "is zero-copy real?" as its own
measurable question.

WHY A LOWER BOUND. Every term is the REAL staged forward cost (T_disp the pjit/XLA dispatch
floor, t_row the staged run_microbatch slope) with the shm tau_io ADDED on top at a
CONSERVATIVE memcpy bandwidth (8 B/ns — a slower copy gives a LARGER tau_io, a LOWER
throughput), B held at a FULL bucket (not the unreachable B->inf asymptote), and the producer
ceiling a hard min taking the worse of the 4.0x/1.9x core scalings. It is a BENCH-dps bound
(the input rates read higher than the closed-loop e2e), contingent on the optimum reaching the
full-bucket feed and on the single-threaded serialization being no worse than the tau_io
charged. EVERY input is resolved through the manifest's TRUST contract and the bound is FLAGGED
when it rests on a seed (trusted=False) rather than a live postgres measurement — ADR-0002: a
seed is never silently reported as measured.
...
SLUG = "shm_spin_poll"
TAU_IO_NAME = "shm_spin_poll_tau_io_us"
WAKEUP_NAME = "shm_spin_poll_wakeup_us"
TMSG_NAME = "shm_spin_poll_tmsg_us_leaf"
REQ_DRAIN_NAME = "shm_spin_poll_req_drain_us"     # the copy-both contrast term (not in the headline cycle)
...
# (the brief names it separately) even though it is ~0 — so the structure makes the spin transport's
# zero-wakeup explicit.
...
# with the WAKEUP term added into the cycle and tau_io being the SHM term.
THROUGHPUT_EXPR = (
    "min("
    "  N_gen*R_gen ,"
...
# carry the baseline registered names; the moved terms carry the prefixed shm names. This map is the model's
# ONLY coupling to the registry — there are no hand-copied literals.
...
_COST: dict[str, float] = {
    ...
    "wakeup": 2.0, "tau_io": 8.0, "t_row": 6.0, "L": 2.0,
}
...
def throughput_numpy(x: dict[str, float]) -> float:
    ...
    SAME formula as THROUGHPUT_EXPR. B is the FULL-bucket width (pad ~= B)."""
...
# --------------------------------------------------------------------------- #
# OpenTURNS / driver surface (mirrors model_cycletime).
# --------------------------------------------------------------------------- #
...
def build_driver(tolerance: float = 5.0, trust: bool = True) -> tuple[Any, dict[str, float]]:
    ...
    Imported lazily (the driver pulls in openturns)."""
...
def cycle_breakdown(x: dict[str, float]) -> dict[str, float]:
    ...
    a caller sees what binds and which term dominates. The shm transport's tau_io + wakeup are the terms it
...
def serve_sawtooth(real: int, buckets=(64, 256, 512), max_batch=512, trust: bool = True) -> float:
    """Serve dps at `real` rows under BUCKETING + the shm tau_io + wakeup terms — the SAWTOOTH (drops at
    bucket edges; peaks at full buckets). Pulls T_disp/tau_io/wakeup/t_row/L from the manifest. Demonstrates
    the non-monotonicity (a larger B is not unconditionally conservative)."""
...
def copy_both_contrast(trust: bool = True) -> dict[str, float]:
    """The PESSIMISTIC arm: charge the request-drain memcpy (`shm_spin_poll_req_drain_us`) on top of the
...
def tmsg_capacity(trust: bool = True) -> dict[str, float]:
    """The transport-stage per-leaf capacity (dps) from `shm_spin_poll_tmsg_us_leaf`, to confirm it is
...
if __name__ == "__main__":
    ...
    print(f"SHM-SPIN-POLL transport — leaf-eval throughput LOWER BOUND")
    ...
    if untrusted:
        print(f"  [ADR-0002] BOUND RESTS ON SEEDS (trusted=False): {untrusted} — flagged, NOT measured. "
              f"Run each bench pinned (taskset -c 0) + sole-workload to flip them to trusted.")
    else:
