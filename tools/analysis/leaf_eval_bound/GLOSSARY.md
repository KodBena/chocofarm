...
| `B` / `B_op` | **rows per forward** (serve batch width / bucket) — the **useful** rows that drive throughput | rows/forward | `B_op` | both |
| `B_eff` / `pad(B)` / `W` | the **compute width** — the *padded* width the forward actually computes on (`≥ B`). The serve cost is `pad(B)·t_row`, **constant in real `B`** up to the pad, and is **distinct from the useful `B`** that drives throughput. Under `--e-policy padmax`, `pad(B)=max_batch` (512, constant — verified live: `CHOCO_EVENTLOG` width=512); under `bucket` it snaps `B` up to ∈{64,256,512}. `pad(B)≈B` **only at a full bucket** — where `throughput_jax` is grounded; `serve_sawtooth(real, max_batch)` is the pad-aware curve for any `B`. Conflating `W` with `B` (calling `throughput_jax` at a non-full `B`) is the mis-evaluation corrected in `static-reconciliation.md`. | rows/forward | (derived `pad(B_op)`) | both |
...
| `T_disp` | **dispatch floor** — pjit/XLA forward-dispatch fixed cost (Design-B's split of the fixed cost). *(A 2026-06-23 note transiently mislabeled a fixed floor `F≈228µs` from a 2-point fit — **retracted**; `T_disp`≈68.84 is the floor, accurate at the W=512 operating point — model compute 2298µs vs measured 2204, ~4%.)* | µs | `T_disp_us` (≈ `DISPATCH_FLOOR_US`) | B |
| `wakeup` | **first-poll readiness** latency, before `tau_io` (Design-B) | µs | `{slug}_wakeup_us` | B (variants) |
| `tau_io` / `T_io` | **server drain / decode / encode / scatter**, serial between forwards. *Currently UNMEASURED — the top measurement target.* Design-A's `INPUT_NAMES` names it `tau_io` (`tau_io_us`); **Design-B's names it `T_io`** — the **same** quantity, not a distinct term. | µs | `tau_io_us` (= `SERVE_IO_US`) | both |
