...
the AllocationDriver owns allocation; bench_store owns SQL).
...
def build_driver(tolerance: float = 5.0, trust: bool = True) -> tuple[Any, dict[str, float]]:
    """Factory: a configured `AllocationDriver` (over the JAX `f` (`throughput_jax`), the per-input costs) + the resolved
...
if __name__ == "__main__":
    ...
    print(f"    (b) VARIANCE-CONTRIBUTION (the allocator's data-driven c_i ranking, once the bound is "
          ...
          f"cycle), so t_row and L (then B) carry the largest a_i=(df/dx)^2*sigma^2 and the AllocationDriver "
